...
{
    //JACOB: TO IMPLEMENT TO SOLVE GRAVITY
...
void CalculatePressure()
{
	...
	for (size_t j=0; j<particleCount; ++j)
		...
		//	if (ratio <= 2.0) {
			...
			// JACOB: IMPLEMENT WENDLAND KERNEL HERE! https://pysph.readthedocs.io/en/latest/reference/kernels.html
			// START WITH WendlandQuintic kernel(C2) kernel for 2D and 3D. 
	       		if (ratio <= 1.0) {
				// For spiky
		...
		//		W = weight * 0.25*pow((2-ratio),3);
			...
			// JACOB: THIS BIT IS FINDING NEIGHBOURS - IT CUTS DOWN YOUR COMPUTATION

		       //printf("Mass: %le Density: %le W: %le kH/10  %lf < r %lf < kH20 %lf \n",pj.m, density, W, kH/10, r, kH*3);
		      if (neighbours[i].count < kMaxNeighbourCount && ratio <=1.0 )
                    	{
...
//Tillotson
// JACOB: PROBABLY ONLY NEED THIS FIRST ONE UNLESS YOU BLOW UP AN ASTEROID
...
//	    double r = sqrt(r2);
	   ...
	   // Using splines
	     ...
	     // For spiky kernel
	     // JACOB TO INCLUDE WENDLAND HERE!
		double weight = 15/(kPi*pow(pi.h,6));
		...
		//	Wgrad = weight*(-3*r/(kH*kH) + (9/4)*(r*r/(kH*kH*kH)));
			...
			// For splines
			//Wgradx = (weight*(-2 + (3/2)*ratio)/(kH*kH)) * (-dx);
			//Wgrady = (weight*(-2 + (3/2)*ratio)/(kH*kH)) * (-dy);
			//Wgradz = (weight*(-2 + (3/2)*ratio)/(kH*kH)) * (-dz);
				}
	    //   else if (ratio > 1.0 && ratio < 2.0) {
		//	W = weight * 0.25*pow((2-ratio),3);
			//Wgrad = 0.25*weight*(-12/kH + 8*r/(kH*kH) - 3*(r*r/(kH*kH*kH)));
	//		Wgradx = (-weight*(3/6)*(2-ratio)*(2-ratio)/kH) * (-dx)/r;
	//		Wgrady = (-weight*(3/6)*(2-ratio)*(2-ratio)/kH) * (-dy)/r;
	//		Wgradz = (-weight*(3/6)*(2-ratio)*(2-ratio)/kH) * (-dz)/r;
	//	       }	    
	    ...
	    I = 0.0;
 	    ...
 	    if (u2 < 0)
            {
      		 mu = (pi.h * u2)/(r*r + 0.001*pi.h*pi.h);
		// beta = 2 for shocks, 0 for viscous accretion
		 I = (1/pi.density) * (-1.5*kC*mu + 2*mu*mu);
...
//	}
	  // printf(" IN here u %.12e kH %le r %.12e mu %.12e kC %le dens %.12e I %.12e \n",u,kH,r,mu,kC,pi.density,I);
	    ...
	    if (pi.h > 0 && pi.density != 0.0 && pj.density != 0.0 && r > 0.0 && ratio < 1.0)
            {
		 Fdx = -1*(pj.m) * (pi.P/((pi.density*pi.density)) + (pj.P/(pj.density*pj.density)) + I)*Wgradx;
		 Fdy = -1*(pj.m) * (pi.P/((pi.density*pi.density)) + (pj.P/(pj.density*pj.density)) + I)*Wgrady;
		 Fdz = -1*(pj.m) * (pi.P/((pi.density*pi.density)) + (pj.P/(pj.density*pj.density)) + I)*Wgradz;
	...
	//	Fdw = -0.5 * (pj.m)*(pi.P/((pi.density*pi.density)) + (pj.P/(pj.density*pj.density))) *dw*Wgradz*1.0;
		...
		pj_txx += pj.txx + pi.m * hxx /pi.density;
		pj_txy += pj.txy + pi.m * hxy /pi.density;
		pj_txz += pj.txz + pi.m * hxz /pi.density;
		pj_tyy += pj.tyy + pi.m * hyy /pi.density;
		pj_tyz += pj.tyz + pi.m * hyz /pi.density;
		pj_tzz += pj.tzz + pi.m * hzz /pi.density;

		
		fViscx = mu*(pi.txx/(pi.density*pi.density) + (pj_txx/(pj.density*pj.density)))*Wgradx + mu*(pi.txy/(pi.density*pi.density) + (pj_txy/(pj.density*pj.density)))*Wgrady + mu*(pi.txz/(pi.density*pi.density) + (pj_txz/(pj.density*pj.density)))*Wgradz;
		fViscy = mu*(pi.tyy/(pi.density*pi.density) + (pj_tyy/(pj.density*pj.density)))*Wgrady + mu*(pi.txy/(pi.density*pi.density) + (pj_txy/(pj.density*pj.density)))*Wgrady + mu*(pi.tyz/(pi.density*pi.density) + (pj_tyz/(pj.density*pj.density)))*Wgradz;
		fViscz = mu*(pi.tzz/(pi.density*pi.density) + (pj_tzz/(pj.density*pj.density)))*Wgradz + mu*(pi.txz/(pi.density*pi.density) + (pj_txz/(pj.density*pj.density)))*Wgradx + mu*(pi.tyz/(pi.density*pi.density) + (pj_tyz/(pj.density*pj.density)))*Wgrady;
 

	     // Add energy components
	    //Viscous entropy
	    pi.tdsdt = (pi.txx*pi.txx + 2*pi.txy*pi.txy + 2*pi.txz*pi.txz + pi.tyy*pi.tyy +2*pi.tyz*pi.tyz + pi.tzz*pi.tzz)*0.5*mu/pi.density;
	    hx = -1*(pi.P/(pi.density*pi.density) + pj.P/(pj.density*pj.density))*Wgradx;	
	    hy = -1*(pi.P/(pi.density*pi.density) + pj.P/(pj.density*pj.density))*Wgrady;	
	    hz = -1*(pi.P/(pi.density*pi.density) + pj.P/(pj.density*pj.density))*Wgradz;	
...
//	}
	...
	//	printf("Hi!\n");
	  ...
	  // JACOB: LEAVE OUT THE VISCOUS STUFF FOR NOW
...
*/
	...
	pi.az = accz;
      	...
      	//pi.energy += (pi.dedt*0.5 +pi.tdsdt);
	pi.dedt *= 0.5;
	pi.dedt += pi.tdsdt;
	pi.energy += pi.dedt*kDt;
	if (pi.energy < 0.0) {
	   pi.energy = 0.0;
	}
...
void CreateParticles(double& avDen, double& maxDen, double& minDen)
{
...
// Read file
  ...
  //  ifstream input("input_77120_2_5_hr.dat");
      //ifstream input("unsteady_b_2hr.dat");
      ifstream input("steady_4hrs.dat");
...
void CreateParticles4(double& avDen, double& maxDen, double& minDen)
{
...
void CoreForm(size_t time, double& avDen, double& maxDen, double& minDen)
{
	size_t count3, count1, count2;
	count1 = 0;
	count2 = 0;
	count3 = 0;
	double t;
	double t_den, low_den, hi_den;
	//t = (time - 1000.0)/2000.0;
	t = time/1000.0;
	//t_den = 5.5e3 + (1 - t)*(6.8e3 - 5.5e3);
	//t_den = 4.5e3 + (1 - t)*(5.7e3 - 4.5e3);
 	t_den = avDen + (1 - t)*(maxDen - avDen);
 	low_den = minDen + 0.1*(maxDen - avDen);
 	hi_den = minDen + 0.6*(maxDen - avDen);
	printf(" ... time %lu t %lf t_den %le (MaxDen %le AvDen %le) \n",time,t,t_den,maxDen,avDen);
       for (int i=0; i<kParticleCount; ++i)
		...
		if (time == 0) {
		 	//if (pi.density < 5.5e3) {
			if (pi.density < hi_den && pi.density > low_den ){
				pi.material = 1;
                                count3++;
			}
			else if (pi.density < 1.09*avDen) {
				pi.material = 0;
				count1++;
			}
			else {
	...
	printf("... Counts: Mantle: %lu Core %lu Core2 %lu \n",count1, count2, count3);
...
*/
    ...
    //readVTK();
    double avDen, maxDen,minDen;
    avDen = 0.0;
    maxDen = 0.0;
    minDen = 0.0;
    //CreateParticles(avDen, maxDen); 
    CreateParticles4(avDen, maxDen, minDen); 
    writeVTK(0);
    printf("Created vtk, \n");
    printf("MaxDen = %le AverageDen = %lei minDen = %le \n",maxDen, avDen, minDen);
    size_t time;
    time = 0;
    size_t dens = 0;
...
//	if (dens < 1 && time > 1000 && time < 2000 ) {
	if (dens < 1 && time < 1000) {
		CoreForm(time, avDen, maxDen, minDen);
		//dens = 1;
	}  
